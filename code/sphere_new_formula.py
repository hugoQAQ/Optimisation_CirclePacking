...
def new_perturb_point(model, gen_pert, delta):
    for i in model.N:
        model.x[i] = model.x[i].value*(1+gen_pert.uniform(-1, 1) * delta)
        model.x[i] = max(model.lb, min(model.x[i].value, model.ub))
        model.y[i] = model.y[i].value * (1 + gen_pert.uniform(-1, 1) * delta)
...
def CirclePacking(size, lb, ub):
    ...
    model.R = pe.Var(bounds=(0.0, 1))
    model.r = pe.Var(model.N, bounds=(0.0, 1))
    model.theta=pe.Var(model.N, bounds=(0.0, 2*pi))
    ...
    def r_rule(model, i):
        if i in model.P:
            return model.r[i] <= 1#1-model.R
        else:
            return pe.Constraint.Skip

    def xy_rule(model, i):
        if i in model.C:
            return model.x[i]**2+model.y[i]**2 <= 1#(1-model.R)**2
        else:
            return pe.Constraint.Skip
    ...
    def cos_rule(model, i, j):
        return model.x[i]==model.r[i]*cos(model.theta[i])

    def sin_rule(model, i, j):
        return model.y[i]==model.r[i]*sin(model.theta[i])

    model.cos_rule = pe.Constraint(model.N, model.N, rule=cos_rule)
    model.sin_rule = pe.Constraint(model.N, model.N, rule=sin_rule)

    def same_radius(model, i, j):
        return model.r[i]==model.r[j]
    #model.same_radius= pe.Constraint(model.N,model.N, rule=same_radius)

    def surface_limit_rule(model):
        return model.R<=(1/model.n*pi)**0.5

    model.surface_limit=pe.Constraint(rule=surface_limit_rule)
...
for algo in ['MBH']: #model.p = model.n-model.c
    for solver in ['knitro', 'snopt']:
        ...
        for i in range(l_n, u_n+1, 5):#range(l_n, u_n+1):
            a+=1
            print("%s Circles"%i)
